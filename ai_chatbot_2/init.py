...
class InitValues:
    def __init__(self):
        ...
        # 데이터
        self.club_info_list = ta.get_clubInfoList()
        self.data_search = ta.get_dateSearch()
        # print("111===============")
        # print(self.club_info_list)
        ...
        self.task_type = ['골프장 예약', '예약', '예약취소', '골프정보'] # task_type을 '예약'1개만 주면 오류가 남
        ...
        initial_reservation_list = [f'예를 들어, "{datetime.now().strftime("%m월 %d일")}에 {initial_golf_list("name")[random.randint(0,len(initial_golf_list("name"))-1)]} 골프장에서 {datetime.now().hour}시 쯤 예약하고 싶어" 라고 하실 수 있습니다.',
                                    f'가입하신 골프장의 지역권은 {(self.comma_delimeter+" ").join(initial_golf_list("location"))} 중에서 선택하시면 됩니다.',
                                    f'골프장의 경우, {(self.comma_delimeter+" ").join(initial_golf_list("name"))} 중에서 선택하실 수 있습니다.',
                                    f'코스는 {(self.comma_delimeter+" ").join(initial_course_list)} 중에서 말씀해주시면 됩니다.']
