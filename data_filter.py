...
class DataFilter:
    def __init__(self):
        self.club_info_list = Tzzim.club_info_list
    ...
    def location_place_clublist(self, location:str, golf_club_name:str):
        ...
        if (location == 'all') and (golf_club_name == 'all'): # 지역, 골프장 모두 상관없음
            club_list = Tzzim.comma_delimeter.join([a_dict['eng_name'] for a_dict in self.club_info_list])
            
        elif (location != 'all') and (golf_club_name == 'all'): # 지역만 조건있고 골프장은 상관없음
            # 복수의 지역
            if Tzzim.or_delimeter in location:
                locations =  location.split(Tzzim.or_delimeter) # [제주도, 영남권]
            ...
            club_list = Tzzim.comma_delimeter.join([club_kor_eng[a_golf_club] for a_golf_club in golf_club_names]) # raon,cypress,ananti

        else: # 지역, 골프장 모두 있는 경우, 골프장만 고려함
            # 복수의 골프장
            if Tzzim.or_delimeter in golf_club_name:
                golf_club_names =  golf_club_name.split(Tzzim.or_delimeter) # [라온, 사이프러스]
                club_list = Tzzim.comma_delimeter.join([club_kor_eng[a_golf_club] for a_golf_club in golf_club_names]) # raon,cypress
            # 하나의 골프장
            else:
    ...
    def course_miniute_processing(self, date_place_result:list[dict], json_condition):
        ...
        for a_dict in date_place_result:
            ...
            if Tzzim.or_delimeter in course: # 복수의 코스
                courses:list = course.split(Tzzim.or_delimeter)
            else:                              # 하나의 코스
                courses:list = [course]
